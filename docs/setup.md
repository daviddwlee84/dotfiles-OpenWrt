...
```sh
wget -O bootstrap.sh https://raw.githubusercontent.com/daviddwlee84/dotfiles-OpenWrt/main/bootstrap.sh
sh bootstrap.sh
```
...
GitHub source snapshot into `~/.local/share/dotfiles-OpenWrt`; it reuses an existing
...
```sh
chezmoi init --apply https://github.com/daviddwlee84/dotfiles-OpenWrt.git
```
...
sh -c "$(curl -fsLS get.chezmoi.io)" -- -b "$HOME/.local/bin" init --apply "https://github.com/$GITHUB_USERNAME/dotfiles-OpenWrt.git"
```
